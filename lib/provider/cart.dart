...
class CartProduct with ChangeNotifier {
  ...
  void addcart(String id, String title, int price) {
    if (_cartproduct.containsKey(id)) {
      _cartproduct.update(
          id,
          (previouscart) => Cart(previouscart.id, previouscart.title,
              previouscart.price, previouscart.quantity + 1));
    } else {
      _cartproduct.putIfAbsent(
          id, () => Cart(DateTime.now().toString(), title, price, 1));
    }
    notifyListeners();
  }
  ...
  void removecartproduct(String productid) {
    ...
    if (_cartproduct[productid].quantity > 1) {
      _cartproduct.update(productid, (existingcart) {
        return Cart(existingcart.id, existingcart.title, existingcart.price,
